...
**Status**: Step 1 CLOSED (2026-06-19), Rows 3+7 live-verified (2026-06-20) — Row 3 ✅ re-verified after S1-MEM-1 (manage_memory confirmed called + store row written); Row 7 ⚠️ re-verified 2026-06-20 with non-empty store — 2 bugs found (store rollback + checkpoint_writes not deleted); Row 4 doc-only (single account); Row 10 + Row 12 doc-only deferred (Sprint 5 / Sprint 2)
...
| **verification_result** | ⚠️ **tool bound, NOT triggered — live bot run 2026-06-20**<br>Bot started locally (`/Users/vernon/project-pantheon`, PID 49182, `claude-haiku`). User sent "remember that my favorite color is blue" via Telegram.<br>```bash<br># Checkpoints before: 3 → after: 6 (bot processed the message)<br>psql -U vernon -d pantheon -c "SELECT thread_id, COUNT(*) FROM checkpoints GROUP BY thread_id;"<br>#  thread_id  | count<br># ------------+-------<br>#  5178700920 |     6<br><br># Store after message:<br>psql -U vernon -d pantheon -c "SELECT prefix, key, value FROM store WHERE value::text ILIKE '%blue%';"<br># → 0 rows (store still empty)<br>```<br>**Root cause**: `agent/prompts.py` `MEMORY_SYSTEM_PROMPT` = "You are a friendly and helpful assistant.\n\n## Memories\n{memory_content}." — no instruction tells the LLM to call `manage_memory` tool. Tool is bound at `agent_factory.py:137` and passed `store=store` at L139, but `claude-haiku` never invoked it.<br>**Verdict (initial)**: ⚠️ tool binding confirmed in code; memory writes DO NOT happen in practice — system prompt missing tool-use instruction.<br>**Verdict (post S1-MEM-1, PR #24 merged 2026-06-20)**: ✅ **VERIFIED**. After prompt fix + `.format()` → `.replace()` hardening + `EMBED_MODEL=none` setup fix, LLM now calls `manage_memory`. Confirmed: `prefix=5178700920, value={"content": "User's favorite color is green"}` written to `store` table. |
| **risk_if_unverified** | ~~Original risk: silent non-functional memory~~ — **RESOLVED by S1-MEM-1 (PR #24)**. |
...
| **verification_result** | ⚠️ **`/reset` partially works — 2 bugs found on re-verification with non-empty store (2026-06-20)**<br><br>**Run 1 (2026-06-20, store=0 pre-reset):** appeared to pass — all 3 tables showed 0 post-reset, but store was already empty so the store DELETE bug was masked.<br><br>**Run 2 (2026-06-20, store=3 pre-reset — re-verified after S1-MEM-1):**<br>```<br># Pre-reset (thread_id = 5178700920):<br>#   store: 3 rows / checkpoints: 6 / checkpoint_writes: (not checked)<br><br># Post-reset:<br>#   store: 3 rows  ← NOT cleared (Bug 1: rollback)<br>#   checkpoints: 0 ← cleared ✅<br>#   checkpoint_writes: 13 ← NOT cleared (Bug 2: missing DELETE)<br>```<br><br>**Bug 1 — store not cleared (transaction rollback):** `db/user_data.py:60-83` puts `DELETE FROM store` and `DELETE FROM store_vectors` in the **same** `async with pool.connection() as conn:` transaction. With `EMBED_MODEL=none`, `store_vectors` table does not exist → exception → psycopg3 ROLLBACK → store DELETE is undone. Log confirms: "Deleted 3 rows from store table" then "Error deleting memories with namespace=5178700920: relation 'store_vectors' does not exist".<br><br>**Bug 2 — checkpoint_writes not cleared:** `db/user_data.py:43-48` only deletes from `checkpoints`; `checkpoint_writes` has no corresponding DELETE. 13 orphaned rows remain after /reset. These orphaned writes are the source of recurring `INVALID_CHAT_HISTORY` errors (LangGraph sees unmatched ToolMessage rows).<br><br>**Bug 3 — no cross-operation atomicity:** Redis, checkpoints, and store are three separate try/except blocks with no compensating rollback. Partial-delete is possible if any step fails.<br><br>**Verdict**: ⚠️ `/reset` clears `checkpoints` only. `store` and `checkpoint_writes` are NOT cleared due to bugs 1 & 2. S1-DEL-1 must fix both before Sprint 1 merge. |
| **risk_if_unverified** | **Bug 1 (store not cleared):** user data persists after explicit `/reset`; user believes their data is gone but LLM continues reading old memories on next session — privacy/trust violation.<br>**Bug 2 (checkpoint_writes not cleared):** orphaned `checkpoint_writes` rows accumulate across resets; each reset that processes a `manage_memory` tool call leaves unmatched ToolMessage rows, causing `INVALID_CHAT_HISTORY: unexpected tool_use_id` on next bot start — UX-breaking crash loop.<br>**Fix target**: `S1-DEL-1` — split store/store_vectors into separate transactions; add `DELETE FROM checkpoint_writes WHERE thread_id = %s` at `db/user_data.py:48`. Re-verified 2026-06-20. |
